...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002030723564').split()] #Channel id for auto indexing ( make sure bot is admin )
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002030723564')) # add premium logs channel id
auth_channel = environ.get('AUTH_CHANNEL', '--1002281015574') #Channel / Group Id for force sub ( make sure bot is admin )
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002030723564').split()]
...
DEENDAYAL_VERIFIED_LOG = int(environ.get('DEENDAYAL_VERIFIED_LOG', '-1002030723564')) #Log channel id ( make sure bot is admin )
